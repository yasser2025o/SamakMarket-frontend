...
<template>
  <div class="shell">
    ...
    <div class="content">
        ...
        <div class="card card-full" style="--delay:.1s">
          ...
          <div class="table-scroll">
            <table class="tbl">
              ...
              <tbody>
                <tr v-for="(v, i) in vendeursFiltres" :key="v.id"
                  class="tbl-row tbl-row-clickable"
                  @click="$router.push('/admin/vendeur/' + v.id)"
                  title="Voir la session de ce vendeur">
                  ...
                  <td class="tbl-last">
                    <span v-if="v.last_login" class="last-time">
                      {{ formatLastLogin(v.last_login) }}
                    </span>
                    <span v-else class="tbl-none">Jamais</span>
                  </td>
                  <td>
      ...
      <div v-show="page === 'tools'" class="page-body">
...
// ── Format date dernière connexion ───────────────────────────
...
//   }
// })
//onst adminStore = useAdminStore()
const chargement = computed(() => adminStore.chargement)
const erreurApi = computed(() => adminStore.erreur)
const vendeurs = computed(() => adminStore.vendeurs)
const produits = computed(() => adminStore.produits)

// d reste reactive mais alimenté par le store
const d = reactive({
totalProduits: 0,
totalPromos: 0,
totalVendeurs: 0,
...
onMounted(async () => {
// Vérifie token avant de charger
if (!authStore.token) return

await adminStore.charger()
...
// onMounted(async () => {
// await adminStore.charger()
// // Les stats viennent toujours de /admin/stats séparément
// // car le store gère vendeurs + produits + flash
// // Si tu as une route /admin/stats garde juste ça :
// try {
// const { data } = await api.get('/admin/stats')
// Object.assign(d, data)
// if (categoriesDispos.value.length) {
...
.tbl-row-clickable:hover td {
  background: rgba(212,175,55,.06) !important;
}
.tbl-row-clickable:hover .vend-name::after {
  content: ' →';
  color: #f5d57a;
  font-size: .7rem;
}
/* Last visit */
.tbl-last  { white-space: nowrap; }
.last-time { color: rgba(255,255,255,.45); font-size: .75rem; }
