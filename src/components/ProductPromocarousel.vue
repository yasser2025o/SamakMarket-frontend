...
<template>
  <section class="promo-section">
    ...
    <div v-else class="slider-outer">
      ...
      <!-- Flèche gauche -->
      <button @click="scrollLeft" class="arrow arrow-left" aria-label="Précédent">‹</button>
      <!-- Flèche droite -->
      <button @click="scrollRight" class="arrow arrow-right" aria-label="Suivant">›</button>
...
// ── Slider manuel : drag souris + touch + flèches ─────────────
...
const startDrag = (e) => {
  ...
  startX      = e.pageX
  scrollStart = sliderRef.value.scrollLeft
}
const onDrag = (e) => {
  if (!isDragging.value) return
  const dx = e.pageX - startX
  sliderRef.value.scrollLeft = scrollStart - dx
}
...
// Boutons flèches
const scrollLeft  = () => { sliderRef.value.scrollBy({ left: -300, behavior: 'smooth' }) }
const scrollRight = () => { sliderRef.value.scrollBy({ left:  300, behavior: 'smooth' }) }
...
/* ══ Slider ═══════════════════════════════════════════════════ */
.slider-outer {
  overflow: hidden; padding: 4px 0 6px;
  position: relative;
}
.promo-track {
...
.promo-track.dragging { cursor: grabbing; scroll-behavior: auto; user-select: none; }
...
.arrow {
  ...
  color: #f5d57a; font-size: 1.4rem; font-weight: 300;
  display: flex; align-items: center; justify-content: center;
  cursor: pointer; transition: all .2s; line-height: 1;
}
.arrow:hover { background: rgba(212,175,55,.3); transform: translateY(-50%) scale(1.1); }
.arrow-left  { left: 6px; }
...
.yt-label    { display: block;text-align: center; color: rgba(203, 238, 6, 0.993); font-family:  Verdana; font-size: .70rem; text-transform: uppercase; letter-spacing: .04em; margin-bottom: 5px; }
...
/* Marocaine */
.yt-maroc {
  background: white; border-color: rgba(212,120,30,.3); color: #c47a1e;
}
.yt-maroc:hover { background: #fff8f0; transform: translateY(-1px); }
/* Méditerranéenne */
.yt-med {
  background: white; border-color: rgba(13,202,240,.3); color: #0a9ab8;
}
.yt-med:hover { background: #f0fbff; transform: translateY(-1px); }
/* ══ WhatsApp ══════════════════════════════════════════════════ */
.btn-wa {
